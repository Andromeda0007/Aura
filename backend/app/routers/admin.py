"""Admin-only: manage admin/teacher/student accounts + cross-batch stats."""
...
class UserCreate(BaseModel):
    ...
    role: UserRole
    semester_ids: list[uuid.UUID] = Field(default_factory=list)
...
class UserUpdate(BaseModel):
    ...
    semester_ids: list[uuid.UUID] | None = None
...
def _set_memberships(db: DBSession, user: User, semester_ids: list[uuid.UUID]) -> None:
    db.query(SemesterMember).filter(SemesterMember.user_id == user.id).delete()
    for sid in dict.fromkeys(semester_ids):  # de-dupe, preserve order
        if db.get(Semester, sid) is None:
            raise HTTPException(status.HTTP_404_NOT_FOUND, f"Semester {sid} not found")
        db.add(SemesterMember(semester_id=sid, user_id=user.id))
...
def _user_dict(db: DBSession, user: User) -> dict:
    sem_ids = [
        str(s)
        for s in db.scalars(
            select(SemesterMember.semester_id).where(SemesterMember.user_id == user.id)
        ).all()
    ]
    return {
        "id": str(user.id),
...
def create_user(
    ...
    _validate_assignment(db, body.role, body.semester_ids)
    user = User(
    ...
    if body.role != UserRole.ADMIN:
        _set_memberships(db, user, body.semester_ids)
    db.commit()
    db.refresh(user)
...
def update_user(
    ...
    if body.semester_ids is not None:
        ...
        _validate_assignment(db, role, body.semester_ids)
...
def admin_stats(db: DBSession = Depends(get_db), _: User = Depends(require_admin)) -> dict:
    ...
    member_counts = dict(
        db.execute(
            select(Department.batch_id, func.count(func.distinct(SemesterMember.user_id)))
            .join(Semester, Semester.department_id == Department.id)
            .join(SemesterMember, SemesterMember.semester_id == Semester.id)
            .group_by(Department.batch_id)
        ).all()
    )
    dept_counts = dict(
        db.execute(select(Department.batch_id, func.count()).group_by(Department.batch_id)).all()
    )
    rows = []
